#!/usr/bin/env python3
"""Debug script to trace RFI extraction."""
...
def trace_rfi(rfi_num: str, project_filter: str = None):
    ...
    for idx, row in df.iterrows():
        ...
        for item in line_items:
            ...
            if rfi_num not in rfi_matches:
                continue

            mentions += 1
            print(f"\nMention #{mentions} - Row {idx}, PCI: {row.get('pci_no', '')}:")
            print(f"  Line item: {item[:200]}...")

            # Extract amounts
            amounts_with_sign = re.findall(r'(-?\d{1,3}(?:,\d{3})*\.\d{2})', item)
            print(f"  All amounts: {amounts_with_sign}")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        print("\n" + "=" * 80)
        trace_rfi("125", "CoreSite Data Center")

        print("\n" + "=" * 80)
        trace_rfi("54", "CoreSite Data Center")
